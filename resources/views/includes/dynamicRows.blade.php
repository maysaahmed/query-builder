<script>
    jQuery(document).ready(function () {
        ...
        $('form').on('click', '.duplicate', function () {
            $('form').append('<div class="row row' + (input) + '"><div class="form-group col-md-3"><select id="col1" name="filter[' + (input) + '][col]" class="form-control col"><option selected>Choose...</option><option value="id">ID</option> <option value="first_name">First Name</option> <option value="last_name">Last Name</option> <option value="full_name">Full Name</option> <option value="gender">Gender</option> <option value="num_msgs">Number of Messages</option> <option value="age">Age</option> <option value="creation_date">Creation Date</option> </select></div>'
                +'<div class="form-group col-md-3"><select id="operator1" name="filter[' + (input) + '][operator]" class="form-control operator"> <option selected>Choose...</option> <option value="=">=</option> <option value="!=">!=</option> <option value=">"> > </option> <option value="<"> < </option> <option value="startswith"> starts with </option> <option value="endswith"> ends with </option> <option value="contains"> contains </option> <option value="exact"> exact </option> </select> </div>'
